...
#define MY_VID 0xFCCF
#define MY_PID 0xF200
...
#define BUF_TX_SIZE 2
#define BUF_RX_SIZE 64
...
int main(void)
{
	...
	char tmp_tx[BUF_TX_SIZE] = {0xa5, 0x50};
	char tmp_rx[BUF_RX_SIZE];
    int ret;
    ...
    ret = usb_bulk_write(dev, EP_OUT, tmp_tx, sizeof(tmp_tx), 5000);
#endif
    if (ret < 0)
    {
    ...
    ret = transfer_bulk_async(dev, EP_IN, tmp_rx, sizeof(tmp_rx), 5000);
#else
    // Running a sync read test
    ret = usb_bulk_read(dev, EP_IN, tmp_rx, sizeof(tmp_rx), 5000);
#endif
    if (ret < 0)
    {
